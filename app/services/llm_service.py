...
class ClinicalLLMService:
    ...
    async def translate_text(
        ...
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            f"If the text is already in {target_name}, return it as is.\n"
                            f"Maintain strict clinical accuracy, preserving medical drug names, dosages, and diagnostic terms.\n"
                            f"Output ONLY the translated text without commentary, preambles, or markdown quotes."
                        ),
                    },
            ...
            raw = resp.choices[0].message.content or text
    ...
    async def translate_clinical_session(
        ...
        target_name = lang_map.get(target_language.lower(), target_language)
        source_name = lang_map.get(source_language.lower(), source_language) if source_language else "the patient's consultation language"
        ...
        system_prompt = (
            ...
            f"If any text or dialogue is already in {target_name}, preserve it faithfully in {target_name}.\n"
            f"CLINICAL RULES:\n"
            ...
            f"4. OUTPUT FORMAT: Output ONLY a valid single JSON object matching this blueprint (no conversational preamble, no markdown, no thoughts):\n"
            f'{{\n'
        ...
        try:
            ...
            # Strip thought tags if generated by thinking models
            cleaned = re.sub(r"<thought>.*?</thought>", "", raw_content, flags=re.DOTALL)
            if "</thought>" in cleaned:
                cleaned = cleaned.split("</thought>")[-1].strip()
            cleaned = re.sub(r"<think>.*?</think>", "", cleaned, flags=re.DOTALL).strip()
